...
export default function FeedPostCard({ post, onCheer }: Props) {
  ...
  return (
    <article className="qp-card p-3.5 space-y-2.5">
      {/* Header */}
      <div className="flex items-center gap-2.5">
        <span className="block w-9 h-9 rounded-full overflow-hidden bg-[var(--qp-inset)] ring-1 ring-[rgba(255,244,228,0.2)] shrink-0">
          <Avatar value={post.userAvatar} size={36} />
        </span>
        <div className="flex-1 min-w-0">
          <p className="qp-title text-[0.8rem] leading-tight truncate">{post.userName}</p>
          <p className="qp-body text-[0.65rem] truncate">
            {quest?.title ?? 'A side quest'} · <span className="qp-num">{relativeTime(post.createdAt)}</span>
          </p>
        </div>
        <span className="qp-badge qp-num text-[0.6rem] shrink-0"
          style={{ color: 'var(--qp-gold)', background: 'rgba(255,182,64,0.1)', borderColor: 'rgba(255,182,64,0.22)' }}>
          <Zap size={9} className="fill-[var(--qp-gold)]" /> +{post.xp}
        </span>
      </div>

      {/* Media */}
      {post.imageUrls.length > 0 && (
        <img src={post.imageUrls[0]} alt="" className="w-full max-h-64 object-cover rounded-[14px]" loading="lazy" />
      )}

      {/* Note */}
      {post.note && <p className="qp-body text-xs leading-relaxed">{post.note}</p>}
      ...
      <div className="flex items-center pt-0.5">
        <button onClick={onCheer}
          ...
          <Heart size={13} className={post.cheeredByMe ? 'fill-[var(--qp-pink-text)]' : ''} />
          <span className="qp-num text-[0.7rem]">{post.cheers}</span>
        </button>
      </div>
    </article>
  )
